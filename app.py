...
# --- Confirmo Tab ---
with tab_confirmo:
    st.header("Confirmo vs Order-list")
    confirmo_file = st.file_uploader("Upload Confirmo File", key="confirmo_file", type=["csv", "xlsx"])
    ...
    if confirmo_file and order_files_confirmo:
        ...
        order_list_dfs = []
        for i, order_file in enumerate(order_files_confirmo):
            if order_file.name.lower().endswith('.csv'):
                df_temp = pd.read_csv(order_file, parse_dates=["Updated At"])
            else:
                df_temp = pd.read_excel(order_file, parse_dates=["Updated At"])
...
with tab_summary:
    ...
    if 'confirmo_summary' in st.session_state:
